...
const Filter = ({
  ...
  useEffect(() => {
    setIsCategoriesItemClosed((prev) => {
      return categories.reduce((state, cat) => {
        ...
        obj[cat.id] =
          cat.subcategories.find((s) => catFilter.has(s.slug)) ||
          prev[cat.id] === false
            ? false
            : true;
        return obj;
  ...
  const slideLeftPosition = `${
    ((priceFilter.get.min || 1) / MAX_PRICE) * 100
  }%`;
  const slideRightPosition = `${
    100 - ((priceFilter.get.max || MAX_PRICE) / MAX_PRICE) * 100
  }%`;
  ...
  const handlePriceSlide = (e) => {
    ...
    if (
      ...
      (!priceFilter.get.max ||
        parseInt(target.value) < parseInt(priceFilter.get.max))
    ) {
      priceFilter.onChange(target.value, "filter-price-min");
    } else if (
      target.id === "price-range-max" &&
      (!priceFilter.get.min ||
        parseInt(target.value) > parseInt(priceFilter.get.min))
    ) {
  ...
  return (
    <aside className={`${styles["catalog__filters"]} `}>
      <div className={styles["filters__categories"]}>
        ...
        <div
          className={`${styles["categories__content"]} ${
              ...
              <div
                className={`${
                  isCategoriesItemClosed[cat.id]
                    ? styles["cat__subcategories_closed"]
                    : styles["cat__subcategories"]
                }`}
              >
                {cat.subcategories.map(
                  (sub) =>
                    locale in sub.translations && (
                      <div
                        key={sub.id}
                        className={styles["subcategories__item"]}
                      >
                        <label>
                          <input
                            name={cat.slug}
                            type="checkbox"
                            value={sub.slug}
                            checked={catFilter.has(sub.slug)}
                            onChange={() => handleCatChange(sub.slug)}
                          />
                          <div
                            className={`${styles["filters__checkbox"]} ${
                              catFilter.has(sub.slug) && styles["checked"]
                            }`}
                          >
